...
public class LeaEngine : BlockCipher
    {
        ...
        private int[] block;
	public LeaEngine() => block = new int[BLOCKSIZE / 4];
	...
	public override void Reset() => Array.Fill(block, 0);
	...
	// utilities
	private static int ROL(int state, int num) => (int)((state << num) | (state >> (32 - num)));

	private static int ROR(int state, int num) => (int)((state >> num) | (state << (32 - num)));
}